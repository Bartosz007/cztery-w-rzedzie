...
def gracz_komputerowy(tpoles, tlevels, diff):
    ...
    fields = right_fields(tpoles, tlevels)
    if diff == 0:
        return losowanie(fields)
    else:
        col = minimax3(ctpoles, ctlevels, diff, True)[0]  # diff jako poziom zagłębienia funckji
        return col


def right_fields(tpoles, tlevels):
    tab = []
...
def losowanie(fields):
    ...
    return fields[rand][1]  # zwraca to krotkę(drugi elelemnt)
...
def minimax3(tpoles, tlevels, depth, maximazing):
    ...
    if maximazing:
        ...
        for row, col in fields:
            ...
            score = minimax3(ctpoles, ctlevels, depth - 1, False)[1]
            if score > value:
                value = score
                column = col
        ...
        for row, col in fields:
            ...
            score = minimax3(ctpoles, ctlevels, depth - 1, True)[1]
            if score < value:
                value = score
                column = col
